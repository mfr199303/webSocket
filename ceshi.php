...
class ceshi
{
   ...
   public function service()
   {
       ...
       $sock = socket_create(AF_INET, SOCK_STREAM, $tcp); //创建一个soekct
       socket_set_option($sock, SOL_SOCKET, SO_REUSEADDR, 1); //对创建好的socket进行设置选项
       if($sock < 0)
       {
          throw new Exception("failed to create socket: ".socket_strerror($sock)."\n");
       }
       socket_bind($sock, $this->address, $this->port); //把创建好的socket绑定在指定的ip和端口上
       socket_listen($sock, $this->port); //开始监听链接
